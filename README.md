...
firstVars = data.frame(INSTRMNT_LANG = rep(1, n), LANG_OTHER = rep(NA, n), GRANT_ID = rep("SP021693", n), DESIGNGRP = rep(1, n), PARTID = HCCPE3monthDat$REDCAP_SURVEY_IDENTIFIER, Date, INTTYPE = rep(3, n), INTDUR = rep(3,n), IINTERVENTION_A = rep(NA, n), INTERVENTION_B = rep(NA, n), INTERVENTION_C = rep(NA, n))
